...
void
print_mov_instruction(std::string_view left, std::string_view right) {
...
Reg
get_reg_from_bits(bool is_word_instruction, uint8_t reg_bits) {
...
int8_t
get_8_bit_disp(const uint8_t *bytes, int instruction_index) {
    return *((int8_t *) (bytes + instruction_index + 2));
}

int16_t
get_16_bit_disp(const uint8_t *bytes, int instruction_index) {
    return *((int16_t *) (bytes + instruction_index + 2));
}

template <typename T>
std::string
disp_to_output_string(T disp) {
    if (disp < 0) {
...
std::string
rm_sring_with_disp(RMNoDisp rm, ModEncoding mod_encoding, const uint8_t *bytes,
    ...
    case ModEncoding::MemoryMode8BitDisplacement: {
        ...
        if (data != 0) {
            rm_field = std::string("[") + std::string(rm_sv) +
                       disp_to_output_string(data) + std::string("]");
        } else {
            rm_field = std::string("[") + std::string(rm_sv) + std::string("]");
        }
        break;
...
std::variant<Reg, RMNoDisp>
get_rm_value(bool is_word_instruction, ModEncoding mod_encoding, uint8_t byte) {
...
int
get_mod_instruction_offset(ModEncoding mod_encoding,
                           std::optional<RMNoDisp> rm) {
...
bool
is_direct_addrress(ModEncoding mod_encoding, RMNoDisp rm) {
    return rm == RMNoDisp::BP &&
           mod_encoding == ModEncoding::MemoryModeNoDisplacement;
}
...
std::string_view
get_id_string(InstructionID id) {
    switch (id) {
...
get_instruction_id(uint8_t byte) {
    if ((byte & 0b11111100) == 0b10001000) {
        ...
        return std::optional{
            InstructionID::MOV_RegisterMemoryToSegmentRegister};
    } else if (byte == 0b10001100) {
        return std::optional{
            InstructionID::MOV_SegmentRegistertoRegisterMemory};
    } else {
        return std::nullopt;
    }
}
...
void
process_instructions(const uint8_t *bytes, uint64_t count) {
    ...
    while (instruction_index < count) {
        ...
        if (opt_id.has_value()) {
            ...
            case InstructionID::MOV_RegisterMemoryToFromRegister: {
                ...
                const Reg reg = get_reg_from_bits(
                    is_word_instruction,
                    (uint8_t) ((next_byte >> 3) & 0b00000111));
                const std::variant<Reg, RMNoDisp> rm =
                    get_rm_value(is_word_instruction, mod_encoding, next_byte);

                int mod_offset;
                if (std::holds_alternative<Reg>(rm)) {
                    const std::string_view rm_reg =
                        reg_string(std::get<Reg>(rm));
                    const std::string_view reg_str = reg_string(reg);
                    print_mov_instruction(rm_reg, reg_str);

                    mod_offset =
                        get_mod_instruction_offset(mod_encoding, std::nullopt);
                } else {
                    ...
                    if (is_direct_addrress(mod_encoding, rm_no_disp)) {
                        int16_t data =
                            get_16_bit_disp(bytes, instruction_index);
                        rm_field = "[" + std::to_string(data) + "]";
                    } else {
                        rm_field = rm_sring_with_disp(rm_no_disp, mod_encoding,
                                                      bytes, instruction_index);
                    }
                    ...
                    mod_offset = get_mod_instruction_offset(
                        mod_encoding, std::optional<RMNoDisp>{rm_no_disp});
                }
            ...
            case InstructionID::MOV_ImmediateToRegister: {
                ...
                const Reg reg = get_reg_from_bits(
                    is_word_instruction, (uint8_t) (byte & 0b00000111));
                const std::string_view reg_str = reg_string(reg);

                if (is_word_instruction) {
                    const int16_t data =
                        *((int16_t *) (bytes + instruction_index + 1));
            ...
            case InstructionID::MOV_ImmediateToRegisterMemory: {
                ...
                if (std::holds_alternative<Reg>(rm)) {
                    ...
                    const RMNoDisp rm_no_disp = std::get<RMNoDisp>(rm);
                    const std::string rm_field = rm_sring_with_disp(
                        rm_no_disp, mod_encoding, bytes, instruction_index);
                ...
                if (is_word_instruction) {
                    const int16_t data =
                        *((int16_t *) (bytes + instruction_index + mod_offset));
                    const std::string data_field =
                        "word " + std::to_string(data);
                    print_mov_instruction(rm_string, data_field);

                    instruction_index += 2;
                } else {
                    const int8_t data =
                        *(bytes + instruction_index + mod_offset);
                    const std::string data_field =
                        "byte " + std::to_string(data);
                    print_mov_instruction(rm_string, data_field);
